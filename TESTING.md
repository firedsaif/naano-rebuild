...
| Tested by | Claude (automated walk of every check) + Saif (sections 1-2) |
| Date / build | 2026-09-12 · commit a5dd647 |
...
| 5.9 | Propose a lower price | Slider and number field work; can't exceed the listed price | ✅ | **Was a bug** (fixed): a typed offer above the listed price stuck, and the button and summary showed it. |
| 5.10 | Send a lower offer | Drawer shows the offer with the listed price struck through | ✅ | **Was a bug** (fixed): a typed amount off the slider's step silently blocked submission, with no message. |
...
| 5.12 | Type an odd amount, e.g. €137, then send | Goes through; never a dead button | ✅ | Added after finding the silent-submit bug |
| 5.13 | A creator already booked on every active campaign | Says so, with a link to create one | ✅ | Added after finding a disabled button with no reason |
...
| 9.2 | Status tabs | All / Active / Draft / Completed filter correctly | ✅ | **Was a bug** (fixed): an invisible overlay from the campaign cards swallowed clicks on these tabs. |
...
| 9.6 | Edit the brief → change something → Save | Change sticks and survives a reload | ✅ | **Was a bug** (fixed): the labels were not tied to their fields. |
...
| 12.8 | Withdraw more than you have | Blocked with a message | ✅ | **Was a bug** (fixed): only the browser's native bubble appeared, not our message. |
...
| 13.2 | Mobile (or a narrow window, ~390px) | No sideways scrolling anywhere | ✅ | **Was a bug** (fixed): /creator scrolled sideways by 33px at 390px. |
...
| 13.11 | Every form control has a name | Nothing unlabelled on any screen (screen-reader check) | ✅ | Added; Radix's hidden inputs are aria-hidden and excluded |
...
| # | Check | Expected | Result | Notes |
| --- | --- | --- | --- | --- |
| 14.1 | Live link in a private window | Loads and is fully usable while signed out | ✅ | All routes return 200 signed out. |
| 14.2 | Live link on your phone | Usable | ☐ | |
| 14.3 | Repo is public | Opens while signed out of GitHub | ✅ | Public. |
| 14.4 | `.agent-logs/` in the repo | Present, with entries through the build | ✅ | Two session logs committed. |
| 14.5 | `CAPTURE-TEST.md` | Present, both canaries pasted | ✅ | Present, both canaries pasted. |
| 14.6 | `npm run lint` | Clean | ✅ | Clean. |
| 14.7 | `npx tsc --noEmit` | Clean | ✅ | Clean. |
| 14.8 | `npm run build` | Succeeds | ✅ | Succeeds. |
| 14.9 | `npx playwright test` | 5 passed | ✅ | 5 passed. |
| 14.10 | `BASE_URL=https://naano-rebuild-six.vercel.app npx playwright test` | 5 passed against live | ✅ | 5 passed against the live URL. |
...
| --- | --- | --- | --- |
| BUG-01 | Campaign cards' invisible overlay swallowed clicks on the status tabs | Major | Fixed (6c43519) |
| BUG-02 | Three forms failed silently when an amount was typed off their step | Major | Fixed (a0f7372) |
| BUG-03 | Creator overview scrolled sideways at 390px | Major on phones | Fixed (6c43519) |
| BUG-04 | A typed offer above the listed price stuck in the UI | Moderate | Fixed (6c43519) |
| BUG-05 | Brief editor labels were not tied to their fields | Moderate (accessibility) | Fixed (6c43519) |
| BUG-06 | Withdrawing over the balance showed only the browser's native bubble | Minor | Fixed (6c43519) |
| BUG-07 | No reason shown when a creator had no bookable campaign left | Minor | Fixed (a5dd647) |

All seven were found by walking this plan, and all are fixed and verified. The
audit re-run after the fixes passed 124 of 124 automated checks.
